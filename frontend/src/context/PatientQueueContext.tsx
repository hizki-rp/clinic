...
const transformPatientData = (backendPatient: any): Patient => {
  return {
    ...
    vitalSigns: backendPatient.vitalSigns,
    triageNotes: backendPatient.triageNotes,
    questioningFindings: backendPatient.questioningFindings,
  ...
  const addPatient = async (patientData: { 
    ...
    try {
      ...
      const patientPayload = {
        ...
        card_number: patientData.card_number || undefined,
        emergency_contact_name: patientData.emergency_contact_name || undefined,
        emergency_contact_phone: patientData.emergency_contact_phone || undefined,
      ...
      // Parse error response to extract specific validation errors
      let errorMessage = 'Failed to add patient';
      
      if (err.message && err.message.includes('HTTP error! status: 400')) {
        try {
          // Extract the error details from the response
          const errorText = err.message.split(' - ')[1];
          if (errorText) {
            const errorData = JSON.parse(errorText);
            if (errorData.card_number && errorData.card_number[0]) {
              errorMessage = errorData.card_number[0];
            }
          }
        } catch (parseError) {
