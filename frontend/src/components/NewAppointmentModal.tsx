...
const NewAppointmentModal: React.FC<NewAppointmentModalProps> = ({ onClose, onSave }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const patientResponse = await fetch('http://localhost:8000/api/healthcare/patients/', {
        ...
        body: JSON.stringify({
          user: {
            first_name: formData.patient_name.split(' ')[0],
            last_name: formData.patient_name.split(' ').slice(1).join(' '),
          },
      ...
      if (appointmentResponse.ok) {
        toast({
          ...
          description: `Appointment scheduled for ${formData.patient_name}`,
        });
        onSave();
      } else {
